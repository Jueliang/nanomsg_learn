 ...
 * `nn_usock_create_io_completion()`, 將 nn_usock::p 關聯到 IOCP。 位於 src/aio/usock_win.inc:539 位於 ；
 * `nn_worker_execute()`，調用 `PostQueuedCompletionStatus()`。 位於 src/aio/worker_win.inc；
 * `nn_worker_routine()`, 調用 `GetQueuedCompletionStatusEx()`。位於 src/aio/worker_win.inc:169: 位於 
 * `nn_worker_term()`, 關閉 IOCP。 位於 src/aio/worker_win.inc:98 位於 `nn_worker_term()`，通知 IOCP 要關閉了。
